...
export const GET = safeApiRoute(
  ...
  async ({ request }, ...args) => {
    ...
    if (supabase) {
      ...
      const key = chatImageObjectKey(name);
      ...
      if (error) {
        logRouteError("GET /api/chat/image/[name] Supabase download", error);
      }
    }

    const localUrl = new URL(`/uploads/chat/${name}`, request.url);
