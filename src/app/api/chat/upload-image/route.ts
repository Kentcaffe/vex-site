...
import { chatImageObjectKey, getSupabaseServiceClient, listingsStorageBucket } from "@/lib/supabase-service-role";
...
export const POST = safeApiRoute("POST /api/chat/upload-image", async ({ request }) => {
  ...
  try {
    ...
    logRouteError("POST /api/chat/upload-image mkdir", error);
  ...
  for (const file of files) {
    ...
    try {
      ...
      logRouteError("POST /api/chat/upload-image file.arrayBuffer", error);
    ...
    const objectKey = chatImageObjectKey(name);
    ...
    if (supabase) {
      ...
      if (error) {
        logRouteError("POST /api/chat/upload-image Supabase Storage", error);
      } else {
        stored = true;
      }
    }
    ...
    if (!stored) {
      ...
      try {
        ...
        logRouteError("POST /api/chat/upload-image writeFile", error);
        return jsonServiceUnavailable("Upload storage is temporarily unavailable.", ApiErrorCode.DATABASE);
      }
    }

    urls.push(`/api/chat/image/${name}`);
  }
