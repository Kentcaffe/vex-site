...
export const GET = safeApiRoute("GET /api/business/document/[name]", async ({ request }, ...args) => {
  ...
  const key = `business-documents/${name}`;
  ...
  if (supabase) {
    ...
    if (error) {
      logRouteError("GET /api/business/document/[name] Supabase download", error);
    }
  }

  const localUrl = new URL(`/uploads/business-documents/${name}`, request.url);
