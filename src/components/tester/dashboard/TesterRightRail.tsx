...
export type TesterRightRailSection = "stats" | "guide" | "leaderboard";

type Props = {
  /** Secțiuni afișate (implicit toate). Folosit pe sub-rute (ex. doar stats sau doar leaderboard). */
  sections?: TesterRightRailSection[];
  guideTitle: string;
  guideSteps: string[];
  statsTitle: string;
...
const DEFAULT_SECTIONS: TesterRightRailSection[] = ["stats", "guide", "leaderboard"];

function showSection(sections: TesterRightRailSection[], id: TesterRightRailSection) {
...
export function TesterRightRail({
  ...
  const sec = sections.length ? sections : DEFAULT_SECTIONS;
  return (
