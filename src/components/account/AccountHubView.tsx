...
export function AccountHubView({ user }: Props) {
  ...
  const firmSlug = user.companyName ? slugifyCompanyName(user.companyName) : "";
    ...
    <div className="app-shell app-section pb-4">
      <header className="mb-6 flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <BusinessBadges isBusiness={isBusiness} isVerified={user.isVerified} className="mt-2" />
          {isBusiness && user.companyName ? (
            <div className="mt-2 flex items-center gap-2">
              ...
              {firmSlug ? (
                <Link href={`/firm/${firmSlug}`} className="truncate text-sm font-semibold text-blue-700 hover:underline">
                  {user.companyName}
                </Link>
              ) : (
                <p className="truncate text-sm font-semibold text-zinc-700">{user.companyName}</p>
              )}
            </div>
          ) : null}
        </div>
      ...
      <nav className="flex flex-col gap-2" aria-label={t("navAria")}>
        ...
        <HubRow href="/upgrade-business" icon={Settings} label="Cont firmă" />
      </nav>
